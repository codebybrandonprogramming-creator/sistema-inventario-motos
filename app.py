...
def nueva_venta():
    ...
    if request.method == "POST":
        ...
        # CÁLCULOS COMPLETOS CON MÁS DECIMALES
...
def historial_ventas():
    ...
    # Procesar ventas y asegurar que todos los campos estén disponibles
    ventas_procesadas = []
    if ventas:
        for v in ventas:
            venta_dict = dict(v)
            # Asegurar que todos los campos numéricos tengan valores por defecto
            venta_dict['iva_unitario'] = venta_dict.get('iva_unitario', 0) or 0
            venta_dict['ganancia_unitaria'] = venta_dict.get('ganancia_unitaria', 0) or 0
            venta_dict['ganancia_total'] = venta_dict.get('ganancia_total', 0) or 0
            venta_dict['porcentaje_ganancia_aplicado'] = venta_dict.get('porcentaje_ganancia_aplicado', 0) or 0
            venta_dict['iva_total'] = venta_dict.get('iva_total', 0) or 0
