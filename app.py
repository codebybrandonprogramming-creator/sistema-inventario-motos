...
def reiniciar_autoincrement_productos():
    ...
    if result and result.get('total', 0) == 0:
        # Si no hay productos, reiniciar el AUTO_INCREMENT a 1
        reset_query = "ALTER TABLE productos AUTO_INCREMENT = 1"
        ejecutar_query(reset_query, commit=True)
        return True
    return False
...
def historial_ventas():
    ...
    query = """
        SELECT 
            ...
            v.ganancia_unitaria,
            v.ganancia_total,
            v.porcentaje_ganancia AS porcentaje_ganancia_aplicado,
            v.usuario_id,
            v.usuario_nombre,
            v.fecha_registro
        FROM ventas v
    ...
    # Convertir a diccionarios comunes para evitar problemas
    ventas = [dict(v) for v in ventas] if ventas else []
...
def eliminar_venta(id):
    ...
    try:
        ...
        # Reiniciar AUTO_INCREMENT si no quedan ventas
